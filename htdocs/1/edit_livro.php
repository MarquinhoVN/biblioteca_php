<?php
    ...
    if($id){
        $livro = array_filter($livros,function($livro) use ($id){
            return $livro->id == $id;
        });
        $livro = array_shift($livro);
    }
?>
			...
			<div class="row justify-content-center">
                <div class="col-md-9">
					<div class="container pagina">
						<div class="row">
							<div class="col">
                                <h4>Livro - ID: <?= $id ?></h4> <hr />
                                <form method="post" action="controller.php?acao=editar&controller=livro&id=<?= $id ?>">
                                    <div class="form-group">
                                        <label>Nome:</label>
                                        <input type="text" name="nome" value="<?= $livro->nome ?>" class="form-control">
                                        <br>
                                        <label>Autor:</label>
                                        <input type="text" name="autor" value="<?= $livro->autor ?>" class="form-control">
										<br>
										<label>Ano:</label>
                                        <input type="text" name="ano" value="<?= $livro->ano ?>" class="form-control">
